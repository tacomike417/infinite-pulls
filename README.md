...
Near the bottom, a card with a National Dex # gets a collapsible **About [Pokémon]** section (`components/pokemon-info.js`, shared by every card detail view in the app — My Collection, Wish List, search results, Other Printings, and a public collector's own card pages) pulling a few fun facts live from the free [PokéAPI](https://pokeapi.co): type(s), region/generation, its evolution family, official artwork, and a button to play its cry. No setup or API key needed. Signed-in visitors also see their own Pokédex building itself automatically from what's actually in My Collection — no manual upkeep: adding any Bulbasaur card marks Bulbasaur "discovered" and shows a running "Your Bulbasaur Collection: N cards" count and evolution-family progress (e.g. "2/3 represented"), all derived from card ownership rather than a separate list to maintain. On a public collector page, this shows the profile owner's Pokédex instead of the visitor's own.
...
From My Account, a visitor can also opt into **Price Alerts** — a push notification when a wish list card drops in price, or a weekly "here's what your collection is worth" update. It runs on a daily schedule server-side (a small Supabase Edge Function + Cron job, no server of your own to run) and reuses the same push notification setup as the shop's banner alerts. See `supabase/SETUP.md` for the one-time setup.
