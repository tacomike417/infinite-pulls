...
Their cards show up as a real "binder" — a 4×4 grid of card art per page, swiped or tapped through horizontally between pages instead of one long scrolling list. Tapping any card opens the same full detail view search results use (prices, rarity, Other Printings, Shop This Card, Recent News, all of it), with a "← Back to My Cards" button to return; a small ✕ on each card removes it without opening anything. Underneath the collection (My Collection tab only) is a **Pokémon News** feed — the latest general Pokémon TCG headlines, same free news source as a single card's Recent News.
