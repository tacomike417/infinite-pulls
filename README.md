...
The detail view also has **Shop This Card** (quick outbound links to eBay, TCGplayer, and Cardmarket) and **Recent News** — real headlines about that card pulled in live via a free news API, not just a link out. See `supabase/SETUP.md` for the one-time setup on the news piece (everything else on this page needs no setup at all).
